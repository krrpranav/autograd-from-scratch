"""Hessian-vector products, without ever building the Hessian.

For a scalar f the Hessian H is n by n, far too large to form for a real network.
But H v (the Hessian along one direction) needs only a couple of passes, and it is
all most second-order methods actually touch: conjugate-gradient Newton, the top
curvature eigenvalue, a Lanczos slice of the spectrum.
...
the Tensor's grad is H v. One forward-over-reverse pass, no Hessian.
...
def top_eigenvalue(f, x, iters=200, seed=0):
    ...
    on H v alone. H is never formed, so this works where np.linalg.eigvalsh cannot."""
...
def _cg(matvec, b, iters, tol):
    """Conjugate gradient for matvec(p) = b. matvec applies H without ever forming it."""
    p = np.zeros_like(b)
    ...
    for _ in range(iters):
        ...
        hd = matvec(d)
        dhd = d @ hd
        if dhd <= 0:  # negative curvature: H is not PD here, so truncate CG (Steihaug)
            return (
                p if p.any() else b
            )  # b = -grad on the first step, still a descent dir
        alpha = rs / dhd
        p = p + alpha * d
        r = r - alpha * hd
        rs_next = r @ r
...
def newton_cg(f, x0, steps=10, cg_iters=20, tol=1e-12):
    """Newton's method that never forms the Hessian. Each step solves H p = -grad for
    the search direction by conjugate gradient on Hessian-vector products alone (the
    Hessian-free approach that scales past toy sizes). CG truncates at negative
    curvature, so this stays finite on indefinite regions; like any Newton method it
    converges to a minimum only where the Hessian is positive definite."""
...
if __name__ == "__main__":
    ...
    hv = hvp(f, x, v)
    hv_dense = hessian(f, x) @ v  # the O(n)-pass route: build all of H, then H @ v
